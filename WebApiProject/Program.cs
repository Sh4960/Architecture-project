...
// =======================
// קריאת מחרוזת החיבור מתוך `appsettings` או משתנה סביבה `DB_CONNECTION`
var connectionString = builder.Configuration.GetConnectionString("Default")
                       ?? Environment.GetEnvironmentVariable("DB_CONNECTION");

if (string.IsNullOrWhiteSpace(connectionString))
{
    // ברירת מחדל זמנית לצורך פיתוח - ניתן לשנות באמצעות appsettings.Development.json
    connectionString = "Data Source=DESKTOP-EODP1E7;Initial Catalog=WebApiDB;Integrated Security=True;Encrypt=True;Trust Server Certificate=True";
}
...
// Apply pending EF Core migrations at startup (useful for Docker/dev)
