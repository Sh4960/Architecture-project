...
namespace WebApiProject.BLL
{
    public class ShoppingBLLService : IShoppingBLLService
    {
        ...
        public ShoppingBLLService(IShoppingDAL shoppingDal, IMapper mapper, IGiftBLLService giftBLL, ILogger<ShoppingBLLService> logger, AppDbContext db)
        {
        ...
        {
            // Begin a transaction to ensure inventory reservation and shopping confirmation are atomic
            using (var tx = await db.Database.BeginTransactionAsync())
            {
                try
                {
                    ...
                    if (!shopping.IsDraft)
                        throw new Exception("Shopping was already confirmed.");

                    // Find inventory record for the gift/product
                    var inv = await db.Inventories.FirstOrDefaultAsync(i => i.GiftId == shopping.GiftId);
                    if (inv == null)
                        throw new Exception("Inventory record not found for product.");

                    if (inv.Quantity < shopping.Quantity)
                        throw new Exception("Not enough inventory to confirm the shopping.");

                    // reserve (decrement) inventory
                    inv.Quantity -= shopping.Quantity;
                ...
                catch (Exception ex)
                {
                    ...
                    logger.LogError(ex, "Error while confirming shopping {Id}", id);
                    throw;
                }
            }
        }
